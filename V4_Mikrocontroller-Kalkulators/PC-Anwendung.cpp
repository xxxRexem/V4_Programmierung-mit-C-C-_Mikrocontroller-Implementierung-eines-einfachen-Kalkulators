...
std::string communicateWithArduino(const std::string& command) {
    ...
    // Wendet die neuen definierten Einstellungen auf die serielle Verbindung an.
    ...
    return response; // Gibt die empfangene Antwort zurück.
}
...
int main()
{
    ...
    while (true)
    {
        displayMenu(); // Greift auf die oben definierte displayMenu Funktion zurück und ruft dies
